...
@Name("clOrgDataModel")
 public class ClOrgDataModel extends BaseDataModel<BaseItem, Long>{
	
	@Logger private static transient Log log;
	
	@In(create=true)
	private ClOrgDataProvider clOrgDataProvider;
	...
	public void walk(FacesContext context, DataVisitor visitor, Range range, Object argument) throws IOException {
		...
		if(this.cachedItems!=null){
		  for (BaseItem itemClOrg:cachedItems) {
			wrappedKeys.add(itemClOrg.getBaseId());
			wrappedData.put(itemClOrg.getBaseId(), itemClOrg);  
			 
		    visitor.process(context, itemClOrg.getBaseId(), argument);
	...
	log.info("auditDataModel:getRowCount:01");
		...
		String  auditListCount = FacesContext.getCurrentInstance().getExternalContext()
			        .getRequestParameterMap()
			        .get("auditListCount");
		log.info("auditDataModel:getRowCount:remoteAudit:"+remoteAuditClOrg);
	...
	protected void resetDataProvider() {
		this.clOrgDataProvider = null;
	}

	public ClOrgDataProvider getDataProvider() {
		  log.info("getDataProvider:01");
		if (clOrgDataProvider == null) {
			log.info("getDataProvider:02");
		}
		return clOrgDataProvider;
	}
