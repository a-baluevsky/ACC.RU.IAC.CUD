...
@Name("armSubDataModel")
 public class ArmSubDataModel extends BaseDataModel<BaseItem, Long>{
	
	@Logger private transient Log log;
	
	@In(create=true)
	private ArmSubDataProvider armSubDataProvider;
	...
	public void walk(FacesContext context, DataVisitor visitor, Range range, Object argument) throws IOException {
		...
		if(this.cachedItems!=null){
		  for (BaseItem itemArmSub:cachedItems) {
			wrappedKeys.add(itemArmSub.getBaseId());
			wrappedData.put(itemArmSub.getBaseId(), itemArmSub);  
			 
		    visitor.process(context, itemArmSub.getBaseId(), argument);
	...
	log.info("auditDataModel:getRowCount:01");
		...
		String  auditListCount = FacesContext.getCurrentInstance().getExternalContext()
			        .getRequestParameterMap()
			        .get("auditListCount");
		log.info("auditDataModel:getRowCount:remoteAudit:"+remoteAuditArmSub);
	 ...
	 public void filterAction() {
		 ...
		 ArmSubStateHolder armSubStateHolder = (ArmSubStateHolder)Component.getInstance("armSubStateHolder", ScopeType.SESSION);
		 armSubStateHolder.clearFilters();
	...
	protected void resetDataProvider() {
		this.armSubDataProvider = null;
	}

	public ArmSubDataProvider getDataProvider() {
		  log.info("getDataProvider:01");
		if (armSubDataProvider == null) {
			log.info("getDataProvider:02");
			}
		return armSubDataProvider;
	}
