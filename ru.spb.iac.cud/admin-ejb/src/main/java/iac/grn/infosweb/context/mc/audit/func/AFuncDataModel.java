...
@Name("aFuncDataModel")
 public class AFuncDataModel extends BaseDataModel<BaseItem, Long>  {
	...
	public void walk(FacesContext context, DataVisitor visitor, Range range, Object argument) throws IOException {
		...
		if(this.cachedItems!=null){
		  for (BaseItem itemAFunc:cachedItems) {
			wrappedKeys.add(itemAFunc.getBaseId());
			wrappedData.put(itemAFunc.getBaseId(), itemAFunc);  
			 
		    visitor.process(context, itemAFunc.getBaseId(), argument);
	...
	log.info("auditDataModel:getRowCount:01");
		...
		String  auditListCount = FacesContext.getCurrentInstance().getExternalContext()
			        .getRequestParameterMap()
			        .get("auditListCount");
		log.info("auditDataModel:getRowCount:remoteAudit:"+remoteAuditAFunc);
		...
		if(this.flagAction==0){
			...
			if (rowCount==null) {
				...
				if(("rowSelectFact".equals(remoteAuditAFunc)/*||
