...
@Name("auditActionDataModel")
 public class AuditActionDataModel extends BaseDataModel<BaseItem, Long> /*extends MCBaseDataModel*/ {
	
	@Logger private transient Log log;
	
	@In(create=true)
	private AuditActionDataProvider auditActionDataProvider;
	...
	public void walk(FacesContext context, DataVisitor visitor, Range range, Object argument) throws IOException {
		...
		if(this.cachedItems!=null){
		  for (BaseItem itemAuditAction:cachedItems) {
			wrappedKeys.add(itemAuditAction.getBaseId());
			wrappedData.put(itemAuditAction.getBaseId(), itemAuditAction);  
			 
		    visitor.process(context, itemAuditAction.getBaseId(), argument);
	...
	log.info("auditDataModel:getRowCount:01");
		...
		String  auditListCount = FacesContext.getCurrentInstance().getExternalContext()
			        .getRequestParameterMap()
			        .get("auditListCount");
		log.info("auditDataModel:getRowCount:remoteAudit:"+remoteAuditAuditAction);
		...
		if(this.flagAction==0){
			...
			if (rowCount==null) {
				...
				if(("rowSelectFact".equals(remoteAuditAuditAction)/*||
	 ...
	 public void filterAction() {
		 ...
		 auditActionStateHolder.clearFilters();
	}
	...
	protected void resetDataProvider() {
		this.auditActionDataProvider = null;
	}

	public AuditActionDataProvider getDataProvider() {
		  log.info("getDataProvider:01");
		if (auditActionDataProvider == null) {
			log.info("getDataProvider:02");
		}
		return auditActionDataProvider;
	}
