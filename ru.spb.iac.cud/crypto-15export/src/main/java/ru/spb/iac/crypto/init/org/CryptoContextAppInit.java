...
import org.w3c.dom.Node;
 ...
 public class CryptoContextAppInit {
	...
	public void fileInit() {
		...
		try {
			...
			for (Node el = configTest6l.getFirstChild(); el != null; el = el
					.getNextSibling()) {
 ...
 if (LOGGER.isDebugEnabled()) {
				...
				if ("KeyResolver".equals(tag)) {
					...
					List classNamesTest6l = new ArrayList(resolverElemTest6l.length);
					for (int i = 0; i < resolverElemTest6l.length; i++) {
						String javaclass = resolverElemTest6l[i].getAttributeNS(null,
								"JAVACLASS");
 ...
 if (LOGGER.isDebugEnabled()) {
		...
		} catch (Exception eTest6l) {
			LOGGER.error("Crypto6l5Init:fileInit:error: ", eTest6l);
		}
	}
