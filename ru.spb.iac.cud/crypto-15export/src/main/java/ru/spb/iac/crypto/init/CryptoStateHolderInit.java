 ...
 public class CryptoStateHolderInit {
	...
	public void fileInit() {
		...
		try {
			...
			for (Node el = configTest5.getFirstChild(); el != null; el = el
					.getNextSibling()) {
...
if (LOGGER.isDebugEnabled()) {
				...
				if ("KeyResolver".equals(tag)) {
					...
					List classNamesTest5 = new ArrayList(resolverElemTest5.length);
					for (int i = 0; i < resolverElemTest5.length; i++) {
						String javaclass = resolverElemTest5[i].getAttributeNS(null,
								"JAVACLASS");
 ...
 if (LOGGER.isDebugEnabled()) {
		...
		} catch (Exception eTest5) {
			LOGGER.error("Crypto55Init:fileInit:error: ", eTest5);
		}
	}
