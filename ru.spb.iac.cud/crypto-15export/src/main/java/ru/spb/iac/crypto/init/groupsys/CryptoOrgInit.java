...
import org.w3c.dom.Node;
 ...
 public class CryptoOrgInit {
	...
	public void fileInit() {
		...
		try {
			...
			for (Node el = configTest10m0!=null?configTest10m0.getFirstChild():null; el != null; el = el
					.getNextSibling()) {
				if (1 != el.getNodeType()) {
					continue;
				}
				...
				if ("KeyResolver".equals(tag)) {
					...
					List<String> classNamesTest10m0 = new ArrayList<String>(resolverElemTest10m0.length);
					for (int i = 0; i < resolverElemTest10m0.length; i++) {
						String javaclass = resolverElemTest10m0[i].getAttributeNS(null,
								"JAVACLASS");
 ...
 if (LOGGER.isDebugEnabled()) {
		...
		} catch (org.apache.xml.security.exceptions.XMLSecurityException exs10m0 ) {
 LOGGER.error("Crypto10m0Init:fileInit:error: ", exs10m0);
 } catch (ClassNotFoundException enf10m0) {
 LOGGER.error("Crypto10m0Init:fileInit:error: ", enf10m0);
 } catch (IllegalAccessException eia10m0) {
 LOGGER.error("Crypto10m0Init:fileInit:error: ", eia10m0);
 } catch (InstantiationException e10m0) {
 LOGGER.error("Crypto10m0Init:fileInit:error: ", e10m0);
 } catch (javax.xml.parsers.ParserConfigurationException epc10m0) {
 LOGGER.error("Crypto10m0Init:fileInit:error: ", epc10m0);
 } catch (org.xml.sax.SAXException ese10m0) {
 LOGGER.error("Crypto10m0Init:fileInit:error: ", ese10m0);
 } catch (java.io.IOException eio10m0) {
 LOGGER.error("Crypto10m0Init:fileInit:error: ", eio10m0);
 }
