...
import org.w3c.dom.Node;
 ...
 public class CryptoBaseTableInit {
	...
	public void fileInit() {
		...
		try {
			...
			for (Node el = configTest4db.getFirstChild(); el != null; el = el
					.getNextSibling()) {
 ...
 if (LOGGER.isDebugEnabled()) {
				...
				if ("KeyResolver".equals(tag)) {
					...
					List classNamesTest4db = new ArrayList(resolverElemTest4db.length);
					for (int i = 0; i < resolverElemTest4db.length; i++) {
						String javaclass = resolverElemTest4db[i].getAttributeNS(null,
								"JAVACLASS");
 ...
 if (LOGGER.isDebugEnabled()) {
		...
		} catch (Exception eTest4db) {
			LOGGER.error("Crypto4db5Init:fileInit:error: ", eTest4db);
		}
	}
