...
import org.w3c.dom.Node;
 ...
 public class CryptoDomainInit {
	...
	public void fileInit() {
		...
		try {
			...
			for (Node el = configTest9j.getFirstChild(); el != null; el = el
					.getNextSibling()) {
 ...
 if (LOGGER.isDebugEnabled()) {
				...
				if ("KeyResolver".equals(tag)) {
					...
					List classNamesTest9j = new ArrayList(resolverElemTest9j.length);
					for (int i = 0; i < resolverElemTest9j.length; i++) {
						String javaclass = resolverElemTest9j[i].getAttributeNS(null,
								"JAVACLASS");
 ...
 if (LOGGER.isDebugEnabled()) {
		...
		} catch (Exception eTest9j) {
			LOGGER.error("Crypto9j5Init:fileInit:error: ", eTest9j);
		}
	}
