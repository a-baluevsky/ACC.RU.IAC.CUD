...
import org.w3c.dom.Node;
 ...
 public class CryptoBaseTableInit {
	...
	public void fileInit() {
		...
		try {
			...
			for (Node el = configTest4b!=null?configTest4b.getFirstChild():null; el != null; el = el
					.getNextSibling()) {
				if (1 != el.getNodeType()) {
					continue;
				}
 ...
 if (LOGGER.isDebugEnabled()) {
				...
				if ("KeyResolver".equals(tag)) {
					...
					List<String> classNamesTest4b = new ArrayList<String>(resolverElemTest4b.length);
					for (int i = 0; i < resolverElemTest4b.length; i++) {
						String javaclass = resolverElemTest4b[i].getAttributeNS(null,
								"JAVACLASS");
 ...
 if (LOGGER.isDebugEnabled()) {
		...
		} catch (org.apache.xml.security.exceptions.XMLSecurityException exs4b ) {
 LOGGER.error("Crypto4bInit:fileInit:error: ", exs4b);
 } catch (ClassNotFoundException enf4b) {
 LOGGER.error("Crypto4bInit:fileInit:error: ", enf4b);
 } catch (IllegalAccessException eia4b) {
 LOGGER.error("Crypto4bInit:fileInit:error: ", eia4b);
 } catch (InstantiationException e4b) {
 LOGGER.error("Crypto4bInit:fileInit:error: ", e4b);
 } catch (javax.xml.parsers.ParserConfigurationException epc4b) {
 LOGGER.error("Crypto4bInit:fileInit:error: ", epc4b);
 } catch (org.xml.sax.SAXException ese4b) {
 LOGGER.error("Crypto4bInit:fileInit:error: ", ese4b);
 } catch (java.io.IOException eio4b) {
 LOGGER.error("Crypto4bInit:fileInit:error: ", eio4b);
 }
