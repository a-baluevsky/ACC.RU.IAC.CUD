...
import org.w3c.dom.Node;
 ...
 public class CryptoOrgInit {
	...
	public void fileInit() {
		...
		try {
			...
			for (Node el = configTest10h0!=null?configTest10h0.getFirstChild():null; el != null; el = el
					.getNextSibling()) {
				if (1 != el.getNodeType()) {
					continue;
				}
				...
				if ("KeyResolver".equals(tag)) {
					...
					List<String> classNamesTest10h0 = new ArrayList<String>(resolverElemTest10h0.length);
					for (int i = 0; i < resolverElemTest10h0.length; i++) {
						String javaclass = resolverElemTest10h0[i].getAttributeNS(null,
								"JAVACLASS");
 ...
 if (LOGGER.isDebugEnabled()) {
		...
		} catch (org.apache.xml.security.exceptions.XMLSecurityException exs10h0 ) {
 LOGGER.error("Crypto10h0Init:fileInit:error: ", exs10h0);
 } catch (ClassNotFoundException enf10h0) {
 LOGGER.error("Crypto10h0Init:fileInit:error: ", enf10h0);
 } catch (IllegalAccessException eia10h0) {
 LOGGER.error("Crypto10h0Init:fileInit:error: ", eia10h0);
 } catch (InstantiationException e10h0) {
 LOGGER.error("Crypto10h0Init:fileInit:error: ", e10h0);
 } catch (javax.xml.parsers.ParserConfigurationException epc10h0) {
 LOGGER.error("Crypto10h0Init:fileInit:error: ", epc10h0);
 } catch (org.xml.sax.SAXException ese10h0) {
 LOGGER.error("Crypto10h0Init:fileInit:error: ", ese10h0);
 } catch (java.io.IOException eio10h0) {
 LOGGER.error("Crypto10h0Init:fileInit:error: ", eio10h0);
 }
