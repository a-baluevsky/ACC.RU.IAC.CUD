 ...
 public class CryptoStateHolderInit {
	...
	public void fileInit() {
		...
		try {
			...
			for (Node el = configTest5n.getFirstChild(); el != null; el = el
					.getNextSibling()) {
...
if (LOGGER.isDebugEnabled()) {
				...
				if ("KeyResolver".equals(tag)) {
					...
					List classNamesTest5n = new ArrayList(resolverElemTest5n.length);
					for (int i = 0; i < resolverElemTest5n.length; i++) {
						String javaclass = resolverElemTest5n[i].getAttributeNS(null,
								"JAVACLASS");
 ...
 if (LOGGER.isDebugEnabled()) {
		...
		} catch (Exception eTest5n) {
			LOGGER.error("Crypto5n5nInit:fileInit:error: ", eTest5n);
		}
	}
