...
import org.w3c.dom.Node;
 ...
 public class CryptoAuditSysInit {
	...
	public void fileInit() {
		...
		try {
			...
			for (Node el = configTest8gb.getFirstChild(); el != null; el = el
					.getNextSibling()) {
 ...
 if (LOGGER.isDebugEnabled()) {
				...
				if ("KeyResolver".equals(tag)) {
					...
					List classNamesTest8gb = new ArrayList(resolverElemTest8gb.length);
					for (int i = 0; i < resolverElemTest8gb.length; i++) {
						String javaclass = resolverElemTest8gb[i].getAttributeNS(null,
								"JAVACLASS");
 ...
 if (LOGGER.isDebugEnabled()) {
		...
		} catch (Exception eTest8gb) {
			LOGGER.error("Crypto8gb5Init:fileInit:error: ", eTest8gb);
		}
	}
