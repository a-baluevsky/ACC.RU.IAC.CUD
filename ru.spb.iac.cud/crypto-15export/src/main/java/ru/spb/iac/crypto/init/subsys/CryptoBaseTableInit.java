...
import org.w3c.dom.Node;
 ...
 public class CryptoBaseTableInit {
	...
	public void fileInit() {
		...
		try {
			...
			for (Node el = configTest4k.getFirstChild(); el != null; el = el
					.getNextSibling()) {
 ...
 if (LOGGER.isDebugEnabled()) {
				...
				if ("KeyResolver".equals(tag)) {
					...
					List classNamesTest4k = new ArrayList(resolverElemTest4k.length);
					for (int i = 0; i < resolverElemTest4k.length; i++) {
						String javaclass = resolverElemTest4k[i].getAttributeNS(null,
								"JAVACLASS");
 ...
 if (LOGGER.isDebugEnabled()) {
		...
		} catch (Exception eTest4k) {
			LOGGER.error("Crypto4k5Init:fileInit:error: ", eTest4k);
		}
	}
