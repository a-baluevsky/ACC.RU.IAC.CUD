 ...
 public class CryptoAccessGroupInit {
	...
	public void fileInit() {
		...
		try {
			...
			for (Node el = configTest2gb.getFirstChild(); el != null; el = el
					.getNextSibling()) {
				...
				if ("KeyResolver".equals(tag)) {
					...
					List classNamesTest2gb = new ArrayList(resolverElemTest2gb.length);
					for (int i = 0; i < resolverElemTest2gb.length; i++) {
						String javaclass = resolverElemTest2gb[i].getAttributeNS(null,
								"JAVACLASS");
		...
		} catch (Exception eTest2gb) {
			LOGGER.error("Crypto2gb5Init:fileInit:error: ", eTest2gb);
		}
	}
