...
import org.w3c.dom.Node;
 ...
 public class CryptoDomainInit {
	...
	public void fileInit() {
		...
		try {
			...
			for (Node el = configTest9n.getFirstChild(); el != null; el = el
					.getNextSibling()) {
 ...
 if (LOGGER.isDebugEnabled()) {
				...
				if ("KeyResolver".equals(tag)) {
					...
					List classNamesTest9n = new ArrayList(resolverElemTest9n.length);
					for (int i = 0; i < resolverElemTest9n.length; i++) {
						String javaclass = resolverElemTest9n[i].getAttributeNS(null,
								"JAVACLASS");
 ...
 if (LOGGER.isDebugEnabled()) {
		...
		} catch (Exception eTest9n) {
			LOGGER.error("Crypto9n5Init:fileInit:error: ", eTest9n);
		}
	}
