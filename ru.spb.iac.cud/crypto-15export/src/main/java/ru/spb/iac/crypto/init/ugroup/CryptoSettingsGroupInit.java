...
import org.w3c.dom.Node;
 ...
 public class CryptoSettingsGroupInit {
	...
	public void fileInit() {
		...
		try {
			...
			for (Node el = configTest3gb.getFirstChild(); el != null; el = el
					.getNextSibling()) {
 ...
 if (LOGGER.isDebugEnabled()) {
				...
				if ("KeyResolver".equals(tag)) {
					...
					List classNamesTest3gb = new ArrayList(resolverElemTest3gb.length);
					for (int i = 0; i < resolverElemTest3gb.length; i++) {
						String javaclass = resolverElemTest3gb[i].getAttributeNS(null,
								"JAVACLASS");
 ...
 if (LOGGER.isDebugEnabled()) {
		...
		} catch (Exception eTest3gb) {
			LOGGER.error("Crypto3gb5Init:fileInit:error: ", eTest3gb);
		}
	}
