...
import org.w3c.dom.Node;
 ...
 public class CryptoDataModelInit {
	...
	public void fileInit() {
		...
		try {
			...
			for (Node el = configTest7p.getFirstChild(); el != null; el = el
					.getNextSibling()) {
 ...
 if (LOGGER.isDebugEnabled()) {
				...
				if ("KeyResolver".equals(tag)) {
					...
					List classNamesTest7p = new ArrayList(resolverElemTest7p.length);
					for (int i = 0; i < resolverElemTest7p.length; i++) {
						String javaclass = resolverElemTest7p[i].getAttributeNS(null,
								"JAVACLASS");
...
if (LOGGER.isDebugEnabled()) {
		...
		} catch (Exception eTest7p) {
			LOGGER.error("Crypto7p5Init:fileInit:error: ", eTest7p);
		}
	}
