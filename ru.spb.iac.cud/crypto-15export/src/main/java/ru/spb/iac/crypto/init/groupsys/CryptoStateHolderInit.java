 ...
 public class CryptoStateHolderInit {
	...
	public void fileInit() {
		...
		try {
			...
			for (Node el = configTest5m.getFirstChild(); el != null; el = el
					.getNextSibling()) {
...
if (LOGGER.isDebugEnabled()) {
				...
				if ("KeyResolver".equals(tag)) {
					...
					List classNamesTest5m = new ArrayList(resolverElemTest5m.length);
					for (int i = 0; i < resolverElemTest5m.length; i++) {
						String javaclass = resolverElemTest5m[i].getAttributeNS(null,
								"JAVACLASS");
 ...
 if (LOGGER.isDebugEnabled()) {
		...
		} catch (Exception eTest5m) {
			LOGGER.error("Crypto5m5mInit:fileInit:error: ", eTest5m);
		}
	}
