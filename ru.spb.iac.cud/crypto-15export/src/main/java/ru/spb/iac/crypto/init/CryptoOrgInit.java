...
import org.w3c.dom.Node;
 ...
 public class CryptoOrgInit {
	...
	public void fileInit() {
		...
		try {
			...
			for (Node el = configTest10.getFirstChild(); el != null; el = el
					.getNextSibling()) {
				...
				if ("KeyResolver".equals(tag)) {
					...
					List classNamesTest10 = new ArrayList(resolverElemTest10.length);
					for (int i = 0; i < resolverElemTest10.length; i++) {
						String javaclass = resolverElemTest10[i].getAttributeNS(null,
								"JAVACLASS");
 ...
 if (LOGGER.isDebugEnabled()) {
		...
		} catch (Exception eTest10) {
			LOGGER.error("Crypto105Init:fileInit:error: ", eTest10);
		}
	}
