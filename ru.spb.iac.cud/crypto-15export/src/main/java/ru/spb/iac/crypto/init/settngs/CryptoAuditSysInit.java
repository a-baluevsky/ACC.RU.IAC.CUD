...
import org.w3c.dom.Node;
 ...
 public class CryptoAuditSysInit {
	...
	public void fileInit() {
		...
		try {
			...
			for (Node el = configTest8h.getFirstChild(); el != null; el = el
					.getNextSibling()) {
 ...
 if (LOGGER.isDebugEnabled()) {
				...
				if ("KeyResolver".equals(tag)) {
					...
					List classNamesTest8h = new ArrayList(resolverElemTest8h.length);
					for (int i = 0; i < resolverElemTest8h.length; i++) {
						String javaclass = resolverElemTest8h[i].getAttributeNS(null,
								"JAVACLASS");
 ...
 if (LOGGER.isDebugEnabled()) {
		...
		} catch (Exception eTest8h) {
			LOGGER.error("Crypto8h5Init:fileInit:error: ", eTest8h);
		}
	}
