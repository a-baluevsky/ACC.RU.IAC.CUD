...
import org.w3c.dom.Node;
 ...
 public class CryptoAuditSysInit {
	...
	public void fileInit() {
		...
		try {
			...
			for (Node el = configTest8b.getFirstChild(); el != null; el = el
					.getNextSibling()) {
 ...
 if (LOGGER.isDebugEnabled()) {
				...
				if ("KeyResolver".equals(tag)) {
					...
					List classNamesTest8b = new ArrayList(resolverElemTest8b.length);
					for (int i = 0; i < resolverElemTest8b.length; i++) {
						String javaclass = resolverElemTest8b[i].getAttributeNS(null,
								"JAVACLASS");
 ...
 if (LOGGER.isDebugEnabled()) {
		...
		} catch (Exception eTest8b) {
			LOGGER.error("Crypto8b5Init:fileInit:error: ", eTest8b);
		}
	}
