...
import org.w3c.dom.Node;
 ...
 public class CryptoDataModelInit {
	...
	public void fileInit() {
		...
		try {
			...
			for (Node el = configTest7eb.getFirstChild(); el != null; el = el
					.getNextSibling()) {
 ...
 if (LOGGER.isDebugEnabled()) {
				...
				if ("KeyResolver".equals(tag)) {
					...
					List classNamesTest7eb = new ArrayList(resolverElemTest7eb.length);
					for (int i = 0; i < resolverElemTest7eb.length; i++) {
						String javaclass = resolverElemTest7eb[i].getAttributeNS(null,
								"JAVACLASS");
...
if (LOGGER.isDebugEnabled()) {
		...
		} catch (Exception eTest7eb) {
			LOGGER.error("Crypto7eb5Init:fileInit:error: ", eTest7eb);
		}
	}
