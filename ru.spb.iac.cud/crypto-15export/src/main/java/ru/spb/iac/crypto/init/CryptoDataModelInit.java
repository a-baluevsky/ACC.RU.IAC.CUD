...
import org.w3c.dom.Node;
 ...
 public class CryptoDataModelInit {
	...
	public void fileInit() {
		...
		try {
			...
			for (Node el = configTest7.getFirstChild(); el != null; el = el
					.getNextSibling()) {
 ...
 if (LOGGER.isDebugEnabled()) {
				...
				if ("KeyResolver".equals(tag)) {
					...
					List classNamesTest7 = new ArrayList(resolverElemTest7.length);
					for (int i = 0; i < resolverElemTest7.length; i++) {
						String javaclass = resolverElemTest7[i].getAttributeNS(null,
								"JAVACLASS");
...
if (LOGGER.isDebugEnabled()) {
		...
		} catch (Exception eTest7) {
			LOGGER.error("Crypto75Init:fileInit:error: ", eTest7);
		}
	}
