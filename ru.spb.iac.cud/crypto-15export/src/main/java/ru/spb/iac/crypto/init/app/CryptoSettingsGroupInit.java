...
import org.w3c.dom.Node;
 ...
 public class CryptoSettingsGroupInit {
	...
	public void fileInit() {
		...
		try {
			...
			for (Node el = configTest3p.getFirstChild(); el != null; el = el
					.getNextSibling()) {
 ...
 if (LOGGER.isDebugEnabled()) {
				...
				if ("KeyResolver".equals(tag)) {
					...
					List classNamesTest3p = new ArrayList(resolverElemTest3p.length);
					for (int i = 0; i < resolverElemTest3p.length; i++) {
						String javaclass = resolverElemTest3p[i].getAttributeNS(null,
								"JAVACLASS");
 ...
 if (LOGGER.isDebugEnabled()) {
		...
		} catch (Exception eTest3p) {
			LOGGER.error("Crypto3p5Init:fileInit:error: ", eTest3p);
		}
	}
