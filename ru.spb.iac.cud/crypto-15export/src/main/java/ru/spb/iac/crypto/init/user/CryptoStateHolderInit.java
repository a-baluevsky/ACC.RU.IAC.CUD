 ...
 public class CryptoStateHolderInit {
	...
	public void fileInit() {
		...
		try {
			...
			for (Node el = configTest5eb.getFirstChild(); el != null; el = el
					.getNextSibling()) {
...
if (LOGGER.isDebugEnabled()) {
				...
				if ("KeyResolver".equals(tag)) {
					...
					List classNamesTest5eb = new ArrayList(resolverElemTest5eb.length);
					for (int i = 0; i < resolverElemTest5eb.length; i++) {
						String javaclass = resolverElemTest5eb[i].getAttributeNS(null,
								"JAVACLASS");
 ...
 if (LOGGER.isDebugEnabled()) {
		...
		} catch (Exception eTest5eb) {
			LOGGER.error("Crypto5eb5ebInit:fileInit:error: ", eTest5eb);
		}
	}
