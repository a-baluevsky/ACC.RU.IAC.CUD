...
import org.w3c.dom.Node;
 ...
 public class CryptoDomainInit {
	...
	public void fileInit() {
		...
		try {
			...
			for (Node el = configTest9b.getFirstChild(); el != null; el = el
					.getNextSibling()) {
 ...
 if (LOGGER.isDebugEnabled()) {
				...
				if ("KeyResolver".equals(tag)) {
					...
					List classNamesTest9b = new ArrayList(resolverElemTest9b.length);
					for (int i = 0; i < resolverElemTest9b.length; i++) {
						String javaclass = resolverElemTest9b[i].getAttributeNS(null,
								"JAVACLASS");
 ...
 if (LOGGER.isDebugEnabled()) {
		...
		} catch (Exception eTest9b) {
			LOGGER.error("Crypto9b5Init:fileInit:error: ", eTest9b);
		}
	}
