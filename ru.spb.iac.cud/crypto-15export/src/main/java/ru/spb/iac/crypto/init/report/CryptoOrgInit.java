...
import org.w3c.dom.Node;
 ...
 public class CryptoOrgInit {
	...
	public void fileInit() {
		...
		try {
			...
			for (Node el = configTest10j0.getFirstChild(); el != null; el = el
					.getNextSibling()) {
				...
				if ("KeyResolver".equals(tag)) {
					...
					List classNamesTest10j0 = new ArrayList(resolverElemTest10j0.length);
					for (int i = 0; i < resolverElemTest10j0.length; i++) {
						String javaclass = resolverElemTest10j0[i].getAttributeNS(null,
								"JAVACLASS");
 ...
 if (LOGGER.isDebugEnabled()) {
		...
		} catch (Exception eTest10j0) {
			LOGGER.error("Crypto10j05Init:fileInit:error: ", eTest10j0);
		}
	}
