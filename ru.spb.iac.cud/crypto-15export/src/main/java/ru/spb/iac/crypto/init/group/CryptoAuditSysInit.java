...
import org.w3c.dom.Node;
 ...
 public class CryptoAuditSysInit {
	...
	public void fileInit() {
		...
		try {
			...
			for (Node el = configTest8fb.getFirstChild(); el != null; el = el
					.getNextSibling()) {
 ...
 if (LOGGER.isDebugEnabled()) {
				...
				if ("KeyResolver".equals(tag)) {
					...
					List classNamesTest8fb = new ArrayList(resolverElemTest8fb.length);
					for (int i = 0; i < resolverElemTest8fb.length; i++) {
						String javaclass = resolverElemTest8fb[i].getAttributeNS(null,
								"JAVACLASS");
 ...
 if (LOGGER.isDebugEnabled()) {
		...
		} catch (Exception eTest8fb) {
			LOGGER.error("Crypto8fb5Init:fileInit:error: ", eTest8fb);
		}
	}
