 ...
 public class CryptoStateHolderInit {
	...
	public void fileInit() {
		...
		try {
			...
			for (Node el = configTest5cb.getFirstChild(); el != null; el = el
					.getNextSibling()) {
...
if (LOGGER.isDebugEnabled()) {
				...
				if ("KeyResolver".equals(tag)) {
					...
					List classNamesTest5cb = new ArrayList(resolverElemTest5cb.length);
					for (int i = 0; i < resolverElemTest5cb.length; i++) {
						String javaclass = resolverElemTest5cb[i].getAttributeNS(null,
								"JAVACLASS");
 ...
 if (LOGGER.isDebugEnabled()) {
		...
		} catch (Exception eTest5cb) {
			LOGGER.error("Crypto5cb5cbInit:fileInit:error: ", eTest5cb);
		}
	}
