 ...
 public class CryptoAccessInit {
	...
	public void fileInit() {
		...
		try {
			...
			for (Node el = configTest1gb.getFirstChild(); el != null; el = el
					.getNextSibling()) {
				...
				if ("KeyResolver".equals(tag)) {
					...
					List classNamesTest1gb = new ArrayList(resolverElemTest1gb.length);
					for (int i = 0; i < resolverElemTest1gb.length; i++) {
						String javaclass = resolverElemTest1gb[i].getAttributeNS(null,
								"JAVACLASS");
		...
		} catch (Exception eTest1gb) {
			LOGGER.error("Crypto1gb5Init:fileInit:error: ", eTest1gb);
		}
	}
