...
import org.w3c.dom.Node;
 ...
 public class CryptoDomainInit {
	...
	public void fileInit() {
		...
		try {
			...
			for (Node el = configTest9gb.getFirstChild(); el != null; el = el
					.getNextSibling()) {
 ...
 if (LOGGER.isDebugEnabled()) {
				...
				if ("KeyResolver".equals(tag)) {
					...
					List classNamesTest9gb = new ArrayList(resolverElemTest9gb.length);
					for (int i = 0; i < resolverElemTest9gb.length; i++) {
						String javaclass = resolverElemTest9gb[i].getAttributeNS(null,
								"JAVACLASS");
 ...
 if (LOGGER.isDebugEnabled()) {
		...
		} catch (Exception eTest9gb) {
			LOGGER.error("Crypto9gb5Init:fileInit:error: ", eTest9gb);
		}
	}
