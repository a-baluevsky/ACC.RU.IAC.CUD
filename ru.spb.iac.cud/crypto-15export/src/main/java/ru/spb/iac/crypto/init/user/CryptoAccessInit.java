 ...
 public class CryptoAccessInit {
	...
	public void fileInit() {
		...
		try {
			...
			for (Node el = configTest1eb.getFirstChild(); el != null; el = el
					.getNextSibling()) {
				...
				if ("KeyResolver".equals(tag)) {
					...
					List classNamesTest1eb = new ArrayList(resolverElemTest1eb.length);
					for (int i = 0; i < resolverElemTest1eb.length; i++) {
						String javaclass = resolverElemTest1eb[i].getAttributeNS(null,
								"JAVACLASS");
		...
		} catch (Exception eTest1eb) {
			LOGGER.error("Crypto1eb5Init:fileInit:error: ", eTest1eb);
		}
	}
