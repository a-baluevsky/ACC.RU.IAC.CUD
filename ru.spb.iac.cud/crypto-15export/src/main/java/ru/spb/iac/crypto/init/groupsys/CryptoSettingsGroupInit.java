...
import org.w3c.dom.Node;
 ...
 public class CryptoSettingsGroupInit {
	...
	public void fileInit() {
		...
		try {
			...
			for (Node el = configTest3m.getFirstChild(); el != null; el = el
					.getNextSibling()) {
 ...
 if (LOGGER.isDebugEnabled()) {
				...
				if ("KeyResolver".equals(tag)) {
					...
					List classNamesTest3m = new ArrayList(resolverElemTest3m.length);
					for (int i = 0; i < resolverElemTest3m.length; i++) {
						String javaclass = resolverElemTest3m[i].getAttributeNS(null,
								"JAVACLASS");
 ...
 if (LOGGER.isDebugEnabled()) {
		...
		} catch (Exception eTest3m) {
			LOGGER.error("Crypto3m5Init:fileInit:error: ", eTest3m);
		}
	}
