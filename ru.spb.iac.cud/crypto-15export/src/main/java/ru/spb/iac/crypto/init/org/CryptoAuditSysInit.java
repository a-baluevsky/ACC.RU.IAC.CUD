...
import org.w3c.dom.Node;
 ...
 public class CryptoAuditSysInit {
	...
	public void fileInit() {
		...
		try {
			...
			for (Node el = configTest8l.getFirstChild(); el != null; el = el
					.getNextSibling()) {
 ...
 if (LOGGER.isDebugEnabled()) {
				...
				if ("KeyResolver".equals(tag)) {
					...
					List classNamesTest8l = new ArrayList(resolverElemTest8l.length);
					for (int i = 0; i < resolverElemTest8l.length; i++) {
						String javaclass = resolverElemTest8l[i].getAttributeNS(null,
								"JAVACLASS");
 ...
 if (LOGGER.isDebugEnabled()) {
		...
		} catch (Exception eTest8l) {
			LOGGER.error("Crypto8l5Init:fileInit:error: ", eTest8l);
		}
	}
