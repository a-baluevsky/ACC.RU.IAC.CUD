 ...
 public class CryptoAccessInit {
	...
	public void fileInit() {
		...
		try {
			...
			for (Node el = configTest1b.getFirstChild(); el != null; el = el
					.getNextSibling()) {
				...
				if ("KeyResolver".equals(tag)) {
					...
					List classNamesTest1b = new ArrayList(resolverElemTest1b.length);
					for (int i = 0; i < resolverElemTest1b.length; i++) {
						String javaclass = resolverElemTest1b[i].getAttributeNS(null,
								"JAVACLASS");
		...
		} catch (Exception eTest1b) {
			LOGGER.error("Crypto1b5Init:fileInit:error: ", eTest1b);
		}
	}
