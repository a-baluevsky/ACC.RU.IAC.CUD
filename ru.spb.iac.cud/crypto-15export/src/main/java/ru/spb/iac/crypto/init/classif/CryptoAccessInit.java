 ...
 public class CryptoAccessInit {
	...
	public void fileInit() {
		...
		try {
			...
			for (Node el = configTest1n.getFirstChild(); el != null; el = el
					.getNextSibling()) {
				...
				if ("KeyResolver".equals(tag)) {
					...
					List classNamesTest1n = new ArrayList(resolverElemTest1n.length);
					for (int i = 0; i < resolverElemTest1n.length; i++) {
						String javaclass = resolverElemTest1n[i].getAttributeNS(null,
								"JAVACLASS");
		...
		} catch (Exception eTest1n) {
			LOGGER.error("Crypto1n5Init:fileInit:error: ", eTest1n);
		}
	}
