 ...
 public class CryptoAccessInit {
	...
	public void fileInit() {
		...
		try {
			...
			for (Node el = configTest1cb.getFirstChild(); el != null; el = el
					.getNextSibling()) {
				...
				if ("KeyResolver".equals(tag)) {
					...
					List classNamesTest1cb = new ArrayList(resolverElemTest1cb.length);
					for (int i = 0; i < resolverElemTest1cb.length; i++) {
						String javaclass = resolverElemTest1cb[i].getAttributeNS(null,
								"JAVACLASS");
		...
		} catch (Exception eTest1cb) {
			LOGGER.error("Crypto1cb5Init:fileInit:error: ", eTest1cb);
		}
	}
