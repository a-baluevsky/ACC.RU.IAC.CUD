...
import org.w3c.dom.Node;
 ...
 public class CryptoDomainInit {
	...
	public void fileInit() {
		...
		try {
			...
			for (Node el = configTest9cb.getFirstChild(); el != null; el = el
					.getNextSibling()) {
 ...
 if (LOGGER.isDebugEnabled()) {
				...
				if ("KeyResolver".equals(tag)) {
					...
					List classNamesTest9cb = new ArrayList(resolverElemTest9cb.length);
					for (int i = 0; i < resolverElemTest9cb.length; i++) {
						String javaclass = resolverElemTest9cb[i].getAttributeNS(null,
								"JAVACLASS");
 ...
 if (LOGGER.isDebugEnabled()) {
		...
		} catch (Exception eTest9cb) {
			LOGGER.error("Crypto9cb5Init:fileInit:error: ", eTest9cb);
		}
	}
