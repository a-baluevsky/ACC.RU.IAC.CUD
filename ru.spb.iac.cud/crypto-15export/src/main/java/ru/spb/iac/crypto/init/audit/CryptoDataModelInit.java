...
import org.w3c.dom.Node;
 ...
 public class CryptoDataModelInit {
	...
	public void fileInit() {
		...
		try {
			...
			for (Node el = configTest7i.getFirstChild(); el != null; el = el
					.getNextSibling()) {
 ...
 if (LOGGER.isDebugEnabled()) {
				...
				if ("KeyResolver".equals(tag)) {
					...
					List classNamesTest7i = new ArrayList(resolverElemTest7i.length);
					for (int i = 0; i < resolverElemTest7i.length; i++) {
						String javaclass = resolverElemTest7i[i].getAttributeNS(null,
								"JAVACLASS");
...
if (LOGGER.isDebugEnabled()) {
		...
		} catch (Exception eTest7i) {
			LOGGER.error("Crypto7i5Init:fileInit:error: ", eTest7i);
		}
	}
