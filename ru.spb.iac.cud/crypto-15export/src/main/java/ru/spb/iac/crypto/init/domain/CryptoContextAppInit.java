...
import org.w3c.dom.Node;
 ...
 public class CryptoContextAppInit {
	...
	public void fileInit() {
		...
		try {
			...
			for (Node el = configTest6db.getFirstChild(); el != null; el = el
					.getNextSibling()) {
 ...
 if (LOGGER.isDebugEnabled()) {
				...
				if ("KeyResolver".equals(tag)) {
					...
					List classNamesTest6db = new ArrayList(resolverElemTest6db.length);
					for (int i = 0; i < resolverElemTest6db.length; i++) {
						String javaclass = resolverElemTest6db[i].getAttributeNS(null,
								"JAVACLASS");
 ...
 if (LOGGER.isDebugEnabled()) {
		...
		} catch (Exception eTest6db) {
			LOGGER.error("Crypto6db5Init:fileInit:error: ", eTest6db);
		}
	}
