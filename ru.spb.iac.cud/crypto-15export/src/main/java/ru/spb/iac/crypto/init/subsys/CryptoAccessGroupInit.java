 ...
 public class CryptoAccessGroupInit {
	...
	public void fileInit() {
		...
		try {
			...
			for (Node el = configTest2k.getFirstChild(); el != null; el = el
					.getNextSibling()) {
				...
				if ("KeyResolver".equals(tag)) {
					...
					List classNamesTest2k = new ArrayList(resolverElemTest2k.length);
					for (int i = 0; i < resolverElemTest2k.length; i++) {
						String javaclass = resolverElemTest2k[i].getAttributeNS(null,
								"JAVACLASS");
		...
		} catch (Exception eTest2k) {
			LOGGER.error("Crypto2k5Init:fileInit:error: ", eTest2k);
		}
	}
