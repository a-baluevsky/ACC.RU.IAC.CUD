package ru.spb.iac.cud.sts.core.auth.util;
 ...
 */
 ...
 public class GOSTAssertionUtil {
	 ...
	 * @throws ProcessingException
	 */
	public static Document asDocument(AssertionType assertion)
			throws ProcessingException {
		ByteArrayOutputStream baos = new ByteArrayOutputStream();
		SAMLAssertionWriter writer = new SAMLAssertionWriter(
				StaxUtil.getXMLStreamWriter(baos));
	...
	public static SAML11AssertionType createSAML11Assertion(String id,
			XMLGregorianCalendar issueInstant, String issuer) {
		SAML11AssertionType assertion = new SAML11AssertionType(id,
				issueInstant);
	 ...
	 *            an object array of attribute values
	 * @return
	 */
	 ...
	 */
	public static void createTimedConditions(AssertionType assertion,
			long durationInMilis, long clockSkew)
	 ...
	 * @throws IssueInstantMissingException
	 */
	public static void createSAML11TimedConditions(
			SAML11AssertionType assertion, long durationInMilis, long clockSkew)
			throws ConfigurationException, IssueInstantMissingException {
		XMLGregorianCalendar issueInstant = assertion.getIssueInstant();
		if (issueInstant == null) {
			throw new IssueInstantMissingException(
	...
	/**
	 * Verify whether the assertion has expired. You can add in a clock skew to
	 * adapt to conditions where in the IDP and SP are out of sync.
	 * 
	 * @param assertion
	 * @param clockSkewInMilis
	 *            in miliseconds
	 * @return
	 * @throws ConfigurationException
	 */
	public static boolean hasExpired(AssertionType assertion,
			long clockSkewInMilis) throws ConfigurationException {
		boolean expiry = false;
		...
		if (conditionsType != null) {
			XMLGregorianCalendar now = XMLTimeUtil.getIssueInstant();
			XMLGregorianCalendar notBefore = conditionsType.getNotBefore();
			XMLGregorianCalendar updatedNotBefore = XMLTimeUtil.subtract(
					notBefore, clockSkewInMilis);
			XMLGregorianCalendar notOnOrAfter = conditionsType
					.getNotOnOrAfter();
	 ...
	 * 
	 * @param assertion
	 *            The {@link AssertionType}
	 * @param roleKeys
	 *            a list of string values representing the role keys. The list
	 *            can be null.
	 * @return
	 */
	public static List<String> getRoles(AssertionType assertion,
		...
		for (StatementAbstractType statement : statements) {
			if (statement instanceof AttributeStatementType) {
				...
				List<ASTChoiceType> attList = attributeStatement
						.getAttributes();
	...
	public static List<String> getRoles(SAML11AssertionType assertion,
			List<String> roleKeys) {
		List<String> roles = new ArrayList<String>();
		List<SAML11StatementAbstractType> statements = assertion
				.getStatements();
