...
import static org.picketlink.common.util.StringUtil.*;
 ...
 public class CUDSAML2AuthenticationHandler extends BaseSAML2Handler  {
	...
	private static final Logger LOGGERSLF4J = LoggerFactory.getLogger(CUDSAML2AuthenticationHandler.class);
    ...
    private class IDPAuthenticationHandler {
        ...
        public void handleRequestType(SAML2HandlerRequest request, SAML2HandlerResponse response) throws ProcessingException {
            ...
            //urn:eis:sys
            String issuer = art.getIssuer().getValue();
            //[urn:eis:sys]http://...
        ...
        public Document getResponse(SAML2HandlerRequest request) throws ConfigurationException, ProcessingException {
            ...
            List<String> resources = (List<String>) session.getAttribute("CUD_RESOURCES");
            
             if ((resources != null && !resources.isEmpty())) {
                
             	 attrStatement = CUDStatementUtil.createAttributeStatementForResources(resources, true);
            ...
            if (attribs != null){
            	...
            	try{
                 ...
                 String lifetime = Long.toString(
                		 conditions.getNotOnOrAfter().toGregorianCalendar().getTimeInMillis());
                 String userUID = (String) attribs.get("USER_UID");
				 ...
				 (new ContextIDPAccessManager())
				   .saveTokenID(base64tokenID, userUID);
            		...
            		throw new ProcessingException("ERROR Generate TOKEN_ID:"+e);
            	}
             }
            ...
            if (attribs != null && attribs.size() > 0) {
            	...
            	if(attrStatement==null){
            	  ...
            	  List <ASTChoiceType> list_attr = attStatement.getAttributes();
            	  
            	  for(ASTChoiceType attribute :list_attr){
