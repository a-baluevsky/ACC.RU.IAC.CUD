 ...
 * Apache XML Security library API. JSR-106 is not yet final. Until that
 * happens,we rely on the non-standard API.
 * 
 * @author
 * @since May 4, 2009
 */
 ...
 public class GOSTXMLEncryptionUtil {
	...
	private static HashMap<String, EncryptionAlgorithm> algorithms = new HashMap<String, EncryptionAlgorithm>(
			4);
	...
	static {
		...
		algorithms.put("aes", new EncryptionAlgorithm("AES", XMLCipher.AES_256,
				256));
	 ...
	 * </p>
	 * <p>
	 * Data is encrypted with a SecretKey. Then the key needs to be transported
	 * to the other end where it is needed for decryption. For the Key
	 * transport, the SecretKey is encrypted with the recipient's public key. At
	 * the receiving end, the receiver can decrypt the Secret Key using his
	 * private key.s
	 * </p>
	 * 
	 * @param document
	 ...
	 * @param keyUsedToEncryptSecretKey
	...
	public static EncryptedKey encryptKey(Document document,
		...
		try {
			String keyWrapAlgo = "urn:ietf:params:xml:ns:cpxmlsec:algorithms:transport-gost2001";
	...
	/**
	 * Given an element in a Document, encrypt the element and replace the
	 * element in the document with the encrypted data
	 * 
	 * @param elementQName
	 *            QName of the element that we like to encrypt
	 * @param publicKey
	 * @param secretKey
	 * @param keySize
	 * @param wrappingElementQName
	 *            A QName of an element that will wrap the encrypted element
	 * @param addEncryptedKeyInKeyInfo
	 *            Need for the EncryptedKey to be placed in ds:KeyInfo
	 * @return
	 * @throws ProcessingException
	 */
	public static void encryptElement(QName elementQName, Document document,
		...
		String encryptionAlgorithm = "urn:ietf:params:xml:ns:cpxmlsec:algorithms:gost28147";
	...
	/**
	 * <p>
	 * Encrypts an element in a XML document using the specified public key,
	 * secret key, and key size. This method doesn't wrap the encrypted element
	 * in a new element. Instead, it replaces the element with its encrypted
	 * version.
	 * </p>
	 * <p>
	 * For example, calling this method to encrypt the <tt><b>inner</b></tt>
	 ...
	 * 
	 * @param document
	 *            the {@code Document} that contains the element to be
	 *            encrypted.
	 * @param element
	 *            the {@code Element} to be encrypted.
	 * @param publicKey
	 *            the {@code PublicKey} that must be used to encrypt the secret
	 *            key.
	 * @param secretKey
	 *            the {@code SecretKey} used to encrypt the specified element.
	 * @param keySize
	 *            the size (in bits) of the secret key.
	 * @throws ProcessingException
	 *             if an error occurs while encrypting the element with the
	 *             specified params.
	 */
	public static void encryptElement(Document document, Element element,
			PublicKey publicKey, SecretKey secretKey, int keySize)
			throws ProcessingException {
		if (element == null) {
		...
		EncryptedKey encryptedKey = encryptKey(document, secretKey, publicKey,
				keySize);
		String encryptionAlgorithm = getXMLEncryptionURL(
				secretKey.getAlgorithm(), keySize);
	...
	/**
	 * Encrypt the root document element inside a Document. <b>NOTE:</> The
	 * document root element will be replaced by the wrapping element.
	 * 
	 * @param document
	 *            Document that contains an element to encrypt
	 * @param publicKey
	 *            The Public Key used to encrypt the secret encryption key
	 * @param secretKey
	 *            The secret encryption key
	 * @param keySize
	 *            Length of key
	 * @param wrappingElementQName
	 *            QName of the element to be used to wrap around the cipher
	 *            data.
	 * @param addEncryptedKeyInKeyInfo
	 *            Should the encrypted key be inside a KeyInfo or added as a
	 *            peer of Cipher Data
	 ...
	 * @return the document with the encrypted element replaced by the data
	 *         element
	...
	public static Element decryptElementInDocument(
		...
		Element encKeyElement = getNextElementNode(encDataElement
				.getNextSibling());
		if (encKeyElement == null) {
			// Search the enc data element for enc key
			NodeList nodeList = encDataElement.getElementsByTagNameNS(
		...
		try {
			...
			encryptedData = cipher.loadEncryptedData(
