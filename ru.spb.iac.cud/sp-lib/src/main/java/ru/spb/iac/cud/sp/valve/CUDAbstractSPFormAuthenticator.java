...
import ru.spb.iac.cud.sp.util.CUDHTTPRedirectUtil;
...
public abstract class CUDAbstractSPFormAuthenticator extends
		BaseFormAuthenticator {

	protected boolean jbossEnv = false;

	public CUDAbstractSPFormAuthenticator() {
		super();
	 ...
	 * @throws IOException
	 */
	protected void sendRequestToIDP(String destination, Document samlDocument,
			String relayState, Response response, boolean willSendRequest,
		...
		if (isHttpPostBinding()) {
			sendHttpPostBindingRequest(destination, samlDocument, relayState,
					response, willSendRequest);
		} else {
			sendHttpRedirectRequest(destination, samlDocument, relayState,
					response, willSendRequest,
	...
	protected void sendHttpRedirectRequest(String destination,
			Document samlDocument, String relayState, Response response,
		...
		if (destinationQueryStringWithSignature != null) {
			...
			destinationQueryString = RedirectBindingUtil
					.getDestinationQueryString(base64Request, relayState,
							willSendRequest);
		}

		RedirectBindingUtilDestHolder holder = new RedirectBindingUtilDestHolder();

		holder.setDestination(destination).setDestinationQueryString(
				destinationQueryString);

		CUDHTTPRedirectUtil.sendRedirectForRequestor(
	...
	protected void sendHttpPostBindingRequest(String destination,
		...
		String samlMessage = PostBindingUtil.base64Encode(DocumentUtil
				.getDocumentAsString(samlDocument));

		DestinationInfoHolder destinationHolder = new DestinationInfoHolder(
				destination, samlMessage, relayState);
	...
	protected void initKeyProvider(Context context) throws LifecycleException {
		...
		try {
			...
			if (clazz == null) {
				throw new ClassNotFoundException(ErrorCodes.CLASS_NOT_LOADED
						+ keyManagerClassName);
			}
			...
			if (authProperties != null) {
				for (AuthPropertyType authPropertyType : authProperties) {
					...
					if (GeneralConstants.X509CERTIFICATE.equals(key)) {
						// we need X509Certificate in SignedInfo. The value is
						// the alias name
						keyManager.addAdditionalOption(
								GeneralConstants.X509CERTIFICATE,
								authPropertyType.getValue());
						break;
					}
				}
			}
			keyManager.addAdditionalOption(
					ServiceProviderBaseProcessor.IDP_KEY,
					new URL(identityURL).getHost());
		} catch (Exception e) {
	 ...
	 * @return
	 * @throws IOException
	 * @throws {@link RuntimeException} when the response is not of type
	 *         catalina response object
	 */
	 ...
	 * 
	 * @see
	 * org.apache.catalina.authenticator.FormAuthenticator#authenticate(org.
	...
	public boolean authenticate(Request request, Response response,
		...
		try {
			...
			// check if this call is resulting from the redirect after
			// successful authentication.
			// if so, make the authentication successful and continue the
			// original request
			if (saveRestoreRequest && matchRequest(request)) {
				logger.trace("Restoring request from session '"
						+ session.getIdInternal() + "'");
				Principal savedPrincipal = (Principal) session
						.getNote(Constants.FORM_PRINCIPAL_NOTE);
				register(request, response, savedPrincipal,
						Constants.LOGIN_METHOD /* .FORM_METHOD */,
						(String) session.getNote(Constants.SESS_USERNAME_NOTE),
						(String) session.getNote(Constants.SESS_PASSWORD_NOTE));

				// try to restore the original request (including post data,
				// etc...)
				if (restoreRequest(request, session)) {
					// success! user is authenticated; continue processing
					// original request
			...
			// If we have already authenticated the user and there is no request
			// from IDP or logout from user
			if (principal != null
			...
			if (StringUtil.isNotNull(spConfiguration.getErrorPage())) {
				try {
					request.getRequestDispatcher(spConfiguration.getErrorPage())
							.forward(request.getRequest(), response);
				} catch (ServletException e1) {
					logger.samlErrorPageForwardError(
							spConfiguration.getErrorPage(), e1);
				}
				return false;
			} else {
				throw e;
	 ...
	 * @throws IOException
	 */
	private boolean handleSAMLRequest(Request request, Response response,
			LoginConfig loginConfig) throws IOException {
		String samlRequest = request
				.getParameter(GeneralConstants.SAML_REQUEST_KEY);
		...
		try {
			...
			// If response is already commited, we need to stop with processing
			// of HTTP request
	 ...
	 * @throws IOException
	 */
	private boolean handleSAMLResponse(Request request, Response response,
			LoginConfig loginConfig) throws IOException {
		Session session = request.getSessionInternal(true);
		String samlResponse = request
				.getParameter(GeneralConstants.SAML_RESPONSE_KEY);
		...
		try {
			ServiceProviderSAMLResponseProcessor responseProcessor = new ServiceProviderSAMLResponseProcessor(
					request.getMethod().equals("POST"), serviceURL,
					this.picketLinkConfiguration);
			if (auditHelper != null) {
			...
			SAML2HandlerResponse saml2HandlerResponse = responseProcessor
					.process(samlResponse, httpContext, handlers, chainLock);

			Document samlResponseDocument = saml2HandlerResponse
			...
			String destinationQueryStringWithSignature = saml2HandlerResponse
			...
			if (destination != null && samlResponseDocument != null) {
				...
				if (principal == null) {
					principal = (Principal) session.getSession().getAttribute(
							GeneralConstants.PRINCIPAL_ID);
				}
				...
				if (logger.isTraceEnabled()) {
					logger.trace("Roles determined for username=" + username
				...
				if (enableAudit) {
					PicketLinkAuditEvent auditEvent = new PicketLinkAuditEvent(
							AuditLevel.INFO);
					auditEvent
							.setType(PicketLinkAuditEventType.RESPONSE_FROM_IDP);
				...
				if (saveRestoreRequest) {
					...
					// Redirect to the original URL. Note that this will trigger
					// the
					// authenticator again, but on resubmission we will look in
					// the
					...
					logger.trace("Redirecting back to original Request URI: "
							+ requestURI);
				...
				register(request, response, principal,
						Constants.LOGIN_METHOD /* Constants.FORM_METHOD */,
						username, password);
				return true;
			}
			...
			if (t != null && t instanceof AssertionExpiredException) {
				...
				if (enableAudit) {
					PicketLinkAuditEvent auditEvent = new PicketLinkAuditEvent(
							AuditLevel.INFO);
					auditEvent
							.setType(PicketLinkAuditEventType.EXPIRED_ASSERTION);
					auditEvent.setAssertionID(((AssertionExpiredException) t)
							.getId());
					auditHelper.audit(auditEvent);
				}
	 ...
	 * @throws IOException
	 */
	private boolean generalUserRequest(Request request, Response response,
			LoginConfig loginConfig) throws IOException {
		Session session = request.getSessionInternal(true);
		boolean willSendRequest = false;
		HTTPContext httpContext = new HTTPContext(request, response,
		...
		try {
			ServiceProviderBaseProcessor baseProcessor = new ServiceProviderBaseProcessor(
					postBinding, serviceURL, this.picketLinkConfiguration);
			if (issuerID != null) {
	...
	/**
	 * Subclasses need to return the context path based on the capability of
	 * their servlet api
	 * 
	 * @return
	 */
